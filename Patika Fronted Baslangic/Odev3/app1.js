...
for (let i = 0; i < menu.length; i++) {
  ...
  <div class="menu-title">
      <h4 style="color:#e00a00">${menu[i].title}</h4>
      <h4 class="price" style="color:#e00a00">$${menu[i].price}</h4>
    </div>
      <div class="menu-text">
      ${menu[i].desc}
      </div>
...
const buttonClick = () => {
  ...
  btn.forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      let menuFilter = menu.filter(menu => menu.category === btnId)
      if (btnId === "All") {
        menuIcerik.innerHTML = ""
        menuIcerik.innerHTML = menu.map(menu => {
          return `
            <div class="menu-items col-sm-6">
              <img class="photo" src="${menu.img}" alt="${menu.title}">
              <div class="menu-info">
              <div class="menu-title">
                    <h4 style="color:#e00a00">${menu.title}</h4>
                    <h4 class="price" style="color:#e00a00">${menu.price}</h4>
                </div>
        ...
        menuIcerik.innerHTML = ""
        menuIcerik.innerHTML = menuFilter.map(menu => {
          return `
          <div class="menu-items col-sm-6">
          <img class="photo" src="${menu.img}" alt="${menu.title}">
              <div class="menu-info">
